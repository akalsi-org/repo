...
| `tools/infra` | `infra` verb dispatcher (Python). Subcommands `adopt`, `status`, `wg-up`, `wg-peer-add`, `vxlan-up`, `hosts-render`, `provision-hetzner`, `decommission`. `deploy` lands with later issues. |
